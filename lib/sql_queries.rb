...
def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
    "SELECT users.name, users.age, SUM(pledges.amount) FROM pledges JOIN users ON users.id = pledges.user_id
    GROUP BY users.name ORDER BY users.name;"
end

def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
    "SELECT projects.title, SUM(pledges.amount)-projects.funding_goal AS amount_over FROM pledges JOIN projects ON projects.id = pledges.project_id GROUP BY projects.title HAVING amount_over >= 0;"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
    "SELECT users.name, SUM(pledges.amount) AS total_pledge FROM pledges JOIN users ON users.id = pledges.user_id GROUP BY users.name ORDER BY total_pledge;"
end
